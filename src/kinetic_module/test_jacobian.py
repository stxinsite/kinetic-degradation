...
def wrap_kinetic_rates(y, params, i):
    """Returns rate of change in species <i>."""
    result = kinetic_rates(y, params)
    return result[i]


def wrap_jac_kinetic_rates(y, params, i):
    """Returns Jacobian of rate of change in species <i>"""
    result = jac_kinetic_rates(y, params)
    return result[i, :]


def wrap_check_grad(x0, params, n_species):
    """Check correctness of provided gradient against a (forward) finite-difference approximation for each species <i>.
    """
...
class MyTestCase(unittest.TestCase):
    def test_jacobian(self):
        ...
        print(grad_err)
        self.assertTrue(np.allclose(a=grad_err, b=0, atol=1e-20, rtol=0.01))
