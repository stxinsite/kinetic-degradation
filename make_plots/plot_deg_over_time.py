...
result_id = f'{test_id}_fix_BPD_ec={bpd_ec}_vary_time_DEG'
result = pd.read_csv(f"./saved_objects/{result_id}.csv")
result.head()

result_deg = result[['t', 'PROTAC', 'degradation', 'all_Ternary', 'Dmax']]
result_deg = result_deg.rename(columns={'degradation': 'Degradation', 'all_Ternary': 'Ternary complex'})
...
plt.title(r'Target Protein Degradation with initial $[PROTAC]_{ec} = 0.0005 \mu$M', y=1.05)

p.tick_params(labelsize=15)
q.tick_params(labelsize=15)
